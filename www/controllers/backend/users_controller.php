...
class users_controller extends controller
{
    public function index()
    {
        if(isset($_POST['delete_btn'])) {
            $this->model('quote_users')->deleteById($_POST['delete_id']);
            header('Location: ' . SITE_DIR . 'users/');
            exit;
        }
        $this->render('users', $this->model('quote_users')->getUsers());
    ...
    public function add()
    {
        if(isset($_POST['save_user_btn'])) {
            ...
            $this->model('quote_users')->insert($row);
            if($_POST['user_password']) {
                $this->logOut();
    ...
    {
        if(isset($_POST['delete_btn'])) {
            $this->model('quote_user_groups')->deleteById($_POST['delete_id']);
            header('Location: ' . SITE_DIR . 'users/groups/');
            exit;
        }
        $this->render('groups', $this->model('quote_user_groups')->getAll());
    ...
    public function add_group()
    {
        ...
        if($_GET['id']) {
            $this->render('group', $this->model('quote_user_groups')->getById($_GET['id']));
        }
    ...
    public function permissions()
    {
        if(isset($_POST['save_permissions_btn'])) {
            $this->model('quote_system_routes_user_groups_relations')->deleteAll();
            foreach($_POST['permission'] as $user_group_id => $routes) {
                if($routes) {
                    foreach($routes as $system_route_id) {
                        ...
                        $this->model('quote_system_routes_user_groups_relations')->insert($row);
                    }
                }
            }
        ...
        $permissions = [];
        $tmp = $this->model('quote_system_routes_user_groups_relations')->getAll();
        foreach($tmp as $v) {
            $permissions[$v['user_group_id']][] = $v['system_route_id'];

        }
        $tmp = $this->model('quote_system_routes')->getByField('permitted', '0', true, 'position');
        $routes = [];
        foreach($tmp as $v) {
            if(!$v['parent']) {
                $routes[$v['id']] = $v;
        ...
        $groups = $this->model('quote_user_groups')->getAll();
