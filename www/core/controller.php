...
//            require_once(!$this->header ? TEMPLATE_DIR . 'common' . DS . 'sidebar.php' : TEMPLATE_DIR . 'common' . DS .$this->sidebar() . '.php');
    ...
    {
        if($_SESSION['auth']) {
            if($user = $this->model('quote_users')->getByFields(array(
                'id' => $_SESSION['user']['id'],
    ...
    private function sidebar()
    {
        ...
        $tmp = $this->model('quote_system_routes_user_groups_relations')->getByField('user_group_id', registry::get('user')['user_group_id'], true);
        $permissions = [];
        foreach($tmp as $v) {
            $permissions[$v['system_route_id']] = 1;
        }
        $sidebar = [];
        $tmp = $this->model('quote_system_routes')->getAll('position');
        $permit_page = false;
        $permitted_urls = [];
